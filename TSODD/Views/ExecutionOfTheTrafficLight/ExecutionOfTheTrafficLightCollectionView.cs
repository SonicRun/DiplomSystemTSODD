...
namespace TSODD.Views.ExecutionOfTheTrafficLightCollectionView{
    public partial class ExecutionOfTheTrafficLightCollectionView : XtraUserControl {
        public ExecutionOfTheTrafficLightCollectionView() {
            InitializeComponent();
			 if(!mvvmContext.IsDesignMode)
                InitBindings();
        }
        void InitBindings() {
            var fluentAPI = mvvmContext.OfType<TSODD.ViewModels.ExecutionOfTheTrafficLightCollectionViewModel>();
            ...
            fluentAPI.WithEvent<GridView, FocusedRowObjectChangedEventArgs>(gridView, "FocusedRowObjectChanged")
                .SetBinding(x => x.SelectedEntity,
                    args => args.Row as TSODD.Entity.Traffic_light.ExecutionOfTheTrafficLight,
