...
namespace TSODD.Views.DegreeOfCapacityStopsCollectionView{
    public partial class DegreeOfCapacityStopsCollectionView : XtraUserControl {
        public DegreeOfCapacityStopsCollectionView() {
            InitializeComponent();
			 if(!mvvmContext.IsDesignMode)
                InitBindings();
        }
        void InitBindings() {
            var fluentAPI = mvvmContext.OfType<TSODD.ViewModels.DegreeOfCapacityStopsCollectionViewModel>();
            ...
            fluentAPI.WithEvent<GridView, FocusedRowObjectChangedEventArgs>(gridView, "FocusedRowObjectChanged")
                .SetBinding(x => x.SelectedEntity,
                    args => args.Row as TSODD.Entity.Public_transport_stop.DegreeOfCapacityStops,
