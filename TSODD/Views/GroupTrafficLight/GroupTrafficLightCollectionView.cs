...
namespace TSODD.Views.GroupTrafficLightCollectionView{
    public partial class GroupTrafficLightCollectionView : XtraUserControl {
        public GroupTrafficLightCollectionView() {
            InitializeComponent();
			 if(!mvvmContext.IsDesignMode)
                InitBindings();
        }
        void InitBindings() {
            var fluentAPI = mvvmContext.OfType<TSODD.ViewModels.GroupTrafficLightCollectionViewModel>();
            ...
            fluentAPI.WithEvent<GridView, FocusedRowObjectChangedEventArgs>(gridView, "FocusedRowObjectChanged")
                .SetBinding(x => x.SelectedEntity,
                    args => args.Row as TSODD.Entity.Traffic_light.GroupTrafficLight,
