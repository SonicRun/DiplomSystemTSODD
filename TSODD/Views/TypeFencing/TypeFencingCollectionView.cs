...
namespace TSODD.Views.TypeFencingCollectionView{
    public partial class TypeFencingCollectionView : XtraUserControl {
        public TypeFencingCollectionView() {
            InitializeComponent();
			 if(!mvvmContext.IsDesignMode)
                InitBindings();
        }
        void InitBindings() {
            var fluentAPI = mvvmContext.OfType<TSODD.ViewModels.TypeFencingCollectionViewModel>();
            ...
            fluentAPI.WithEvent<GridView, FocusedRowObjectChangedEventArgs>(gridView, "FocusedRowObjectChanged")
                .SetBinding(x => x.SelectedEntity,
                    args => args.Row as TSODD.Entity.Fencing.TypeFencing,
