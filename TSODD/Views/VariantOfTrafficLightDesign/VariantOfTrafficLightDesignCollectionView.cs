...
namespace TSODD.Views.VariantOfTrafficLightDesignCollectionView{
    public partial class VariantOfTrafficLightDesignCollectionView : XtraUserControl {
        public VariantOfTrafficLightDesignCollectionView() {
            InitializeComponent();
			 if(!mvvmContext.IsDesignMode)
                InitBindings();
        }
        void InitBindings() {
            var fluentAPI = mvvmContext.OfType<TSODD.ViewModels.VariantOfTrafficLightDesignCollectionViewModel>();
            ...
            fluentAPI.WithEvent<GridView, FocusedRowObjectChangedEventArgs>(gridView, "FocusedRowObjectChanged")
                .SetBinding(x => x.SelectedEntity,
                    args => args.Row as TSODD.Entity.Traffic_light.VariantOfTrafficLightDesign,
