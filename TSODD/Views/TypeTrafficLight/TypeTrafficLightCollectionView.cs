...
namespace TSODD.Views.TypeTrafficLightCollectionView{
    public partial class TypeTrafficLightCollectionView : XtraUserControl {
        public TypeTrafficLightCollectionView() {
            InitializeComponent();
			 if(!mvvmContext.IsDesignMode)
                InitBindings();
        }
        void InitBindings() {
            var fluentAPI = mvvmContext.OfType<TSODD.ViewModels.TypeTrafficLightCollectionViewModel>();
            ...
            fluentAPI.WithEvent<GridView, FocusedRowObjectChangedEventArgs>(gridView, "FocusedRowObjectChanged")
                .SetBinding(x => x.SelectedEntity,
                    args => args.Row as TSODD.Entity.Traffic_light.TypeTrafficLight,
