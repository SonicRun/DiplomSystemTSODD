...
namespace TSODD.Views.TypeOfLayoutStopCollectionView{
    public partial class TypeOfLayoutStopCollectionView : XtraUserControl {
        public TypeOfLayoutStopCollectionView() {
            InitializeComponent();
			 if(!mvvmContext.IsDesignMode)
                InitBindings();
        }
        void InitBindings() {
            var fluentAPI = mvvmContext.OfType<TSODD.ViewModels.TypeOfLayoutStopCollectionViewModel>();
            ...
            fluentAPI.WithEvent<GridView, FocusedRowObjectChangedEventArgs>(gridView, "FocusedRowObjectChanged")
                .SetBinding(x => x.SelectedEntity,
                    args => args.Row as TSODD.Entity.Public_transport_stop.TypeOfLayoutStop,
