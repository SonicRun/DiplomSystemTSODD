...
namespace TSODD.Views.SubgroupFencingCollectionView{
    public partial class SubgroupFencingCollectionView : XtraUserControl {
        public SubgroupFencingCollectionView() {
            InitializeComponent();
			 if(!mvvmContext.IsDesignMode)
                InitBindings();
        }
        void InitBindings() {
            var fluentAPI = mvvmContext.OfType<TSODD.ViewModels.SubgroupFencingCollectionViewModel>();
            ...
            fluentAPI.WithEvent<GridView, FocusedRowObjectChangedEventArgs>(gridView, "FocusedRowObjectChanged")
                .SetBinding(x => x.SelectedEntity,
                    args => args.Row as TSODD.Entity.Fencing.SubgroupFencing,
